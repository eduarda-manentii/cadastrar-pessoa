...
bool sqlite3_reader::isnull(int index) {
	if(!this->cmd) throw database_error("reader is closed");
	if((index)>(this->cmd->argc-1)) throw std::out_of_range("index out of range");
	return sqlite3_column_type(this->cmd->stmt, index) == SQLITE_NULL;
}

int sqlite3_reader::columncount(){
    return this->cmd->argc;
}


template <>
void sqlite3_reader::get(int index, int& var){
    var = getint(index);
}

template <>
void sqlite3_reader::get(int index, double& var){
    var = getdouble(index);
}

template<>
void sqlite3_reader::get(int index, std::string& var){
    var = getstring(index);
}
