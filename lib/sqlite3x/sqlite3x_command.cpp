...
void sqlite3_command::bind(int index, const char *data) {
	if(sqlite3_bind_text(this->stmt, index, data, std::strlen(data), SQLITE_TRANSIENT)!=SQLITE_OK)
		throw database_error(this->con);
}
...
long long sqlite3_command::insertid(){
    return con.insertid();
}
